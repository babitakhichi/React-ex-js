...
function JitsiMeet() {
  ...
  let timer;
  const subscriptionDataActive = getCurrentActiveSubscription(
  ...
  useEffect(() => {
    if (userActiveSubscription?.length > 0) {
      ...
      let features = subscriptionpPlanFormetter(
        subscriptionData?.Subscription?.SubscriptionFeature
      );
  ...
  const updateMeetingRoomStatus = async (data) => {
    try {
      ...
      if (success === 1) {
        ...
        if (!subscriptionDataActive.is_base) {
          room.meetingData.end_max_date = agoDateTime(
            room?.meetingData?.max_meet_duration ?? meetingDuration,
            "minutes",
            "YYYY-MM-DD HH:mm:ss",
            startDate
          );
  ...
  }, [roomData, roomData?.meetingData?.end_max_date]);
    ...
    try {
      let queryParams = {
        start_date: momentTimezoneFormatter(new Date())
          .utc()
          .format("YYYY-MM-DD"),
        is_corporate: getIscorporateActive(accounts),
        corporate_id: getActiveAccount(accounts)?.id,
      };
      ...
      {loading ? (
        ...
        startTime ? (
          ...
          <JitsiMeeting
            ...
            userInfo={{
              displayName: `${userData?.UserProfile?.full_name}`,
            }}
            configOverwrite={{
