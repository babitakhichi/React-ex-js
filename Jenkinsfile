    ...
    stages {
        ...
        stage('Copy Artifacts') {
           steps {
            ...
            script {
                 sh """
                  sudo rsync -rv --mkpath -e 'ssh -i ${KEY} -p 22 -o StrictHostKeyChecking=no' ${DOCKER_PATH}/build/* ${USERNAME}@${IP_ADDRESS}:${PROJECT_PATH}${PROJECT_NAME}-${env.BRANCH_NAME}
                 """
                }
            }
        }
